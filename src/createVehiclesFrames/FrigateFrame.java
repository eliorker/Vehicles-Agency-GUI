...
import vehiclesClasses.Frigate;
...
public class FrigateFrame extends JFrame {
	...
	public FrigateFrame() {
		image1 =new ImageIcon(getClass().getResource("frigate1.jpg")); // defaulty
		...
		topPanel.add(label1 = new JLabel("Enter The Frigate Details"));
		...
		middlePanel.add(label4 = new JLabel("Model:"));
		middlePanel.add(text3 = new JTextArea("Model"));
		text3.setBorder(border);
		middlePanel.add(label5 = new JLabel("Wind direction:"));
		middlePanel.add(text4 = new JTextArea("with the wind / against the wind"));
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == button1) { // add button
				if(text1.getText().equals("") || text2.getText().equals("")|| text3.getText().equals("")|| text4.getText().equals("")) {
					...
					try {
						boolean flag=false;
						int maxSpeed =Integer.parseInt(text2.getText())  , maxPassenger=Integer.parseInt(text1.getText());
						String model=text3.getText() , windDirecrion = text4.getText();
						flag = true;
						if(flag) {
							JFrame SuccedOrNot = CreateFrame("Vehicle Added successfully ") ;
						}
						Frigate frigate = new Frigate(maxPassenger , maxSpeed , model , windDirecrion  );
						mainApp.addImages(image1);
						mainApp.getVehicles().add(frigate);
						mainApp.getWaterVehicles().add(frigate);
	...
	private class ListenForRadioButton implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == Rbutton1) {
				image1 =new ImageIcon(getClass().getResource("frigate1.jpg"));
				label6.setIcon(image1);
				//Rbutton1.setSelected(false);
				}
			else if (e.getSource() == Rbutton2) {
				image1 =new ImageIcon(getClass().getResource("frigate2.jpg"));
				label6.setIcon(image1);
				}
			else // (e.getSource() == Rbutton3) 
				{
				image1 =new ImageIcon(getClass().getResource("frigate3.jpg"));
				label6.setIcon(image1);
				}
