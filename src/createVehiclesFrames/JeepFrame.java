...
public class JeepFrame extends JFrame {
	...
	public JeepFrame() {
		image1 =new ImageIcon(getClass().getResource("jeep1.jpg")); // defaulty
		...
		topPanel.add(label1 = new JLabel("Enter The Jeep Details"));
		...
		middlePanel.add(label4 = new JLabel("Average Fuel Consumption:"));
		middlePanel.add(text3 = new JTextArea("Average Fuel Consumption"));
		text3.setBorder(border);
		middlePanel.add(label5 = new JLabel("Engine Life:"));
		middlePanel.add(text4 = new JTextArea("Engine Life"));
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == button1) { // add button
				if(text1.getText().equals("") || text2.getText().equals("")|| text3.getText().equals("")|| text4.getText().equals("")) {
					...
					try {
						boolean flag=false;
						int speed = Integer.parseInt(text1.getText()), engineLife =Integer.parseInt(text4.getText()) ;
						String model = text2.getText(); 
						float AverageFuelConsumption =Float.parseFloat(text3.getText());
						flag = true;
						if(flag) {
							JFrame SuccedOrNot = CreateFrame("Vehicle Added successfully ") ;
						}
						Jeep jeep = new Jeep(speed ,model , AverageFuelConsumption ,engineLife );
						mainApp.addImages(image1);
						mainApp.getVehicles().add(jeep);
	...
	private class ListenForRadioButton implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == Rbutton1) {
				image1 =new ImageIcon(getClass().getResource("jeep1.jpg"));
				label6.setIcon(image1);
				//Rbutton1.setSelected(false);
				}
			else if (e.getSource() == Rbutton2) {
				image1 =new ImageIcon(getClass().getResource("jeep2.jpg"));
				label6.setIcon(image1);
				}
			else  //(e.getSource() == Rbutton3)
				{
				image1 =new ImageIcon(getClass().getResource("jeep3.jpg"));
				label6.setIcon(image1);
				}
